...
for problem_num, (start_col, end_col) in enumerate(problem_ranges):
    ...
    operator_line = problem_data[-1].strip()
    operator = operator_line[-1] if operator_line else None
    
    if not operator or operator not in ['+', '*']:
        print(f"Problem {problem_num}: Skipping - operator is '{operator}' (line: '{operator_line}')")
        continue
...
print(f"Processed {len(answers)} problems")
